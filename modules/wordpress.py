...
aweber-web-form-widget
    ...
    try:    
        sleep(0.1)
        print(color.CYAN+"[!] Enter A website Based on Wordpress\n")
        sleep(0.1)
        print(color.CYAN+"[*] Please Do not Include Http in the url\n")
        sleep(0.1)
        site = input(color.RED+"┌─["+color.LIGHTGREEN_EX+"WebRobber"+color.RED+"/"+color.WHITE+"Home"+color.RED+"/"+color.CYAN+"CMS"+color.RED+"/"+color.LIGHTYELLOW_EX+"WP"+color.RED+"/"+color.CYAN+"Plugins-FInder"+"""]
└──╼ """+color.WHITE+"» ")

        if site == '' or None:
            sleep(1)
            print(color.RED+"Please Enter The Url")
            sleep(0.3)
            sys.exit()

        data = requests.get('http://'+site+'/wp-content/plugins/')
        if data.status_code == 404 or data.status_code == 500 or data.status_code == 403:
            print(color.RED+" [!]"+color.YELLOW+'The Website is not based on Wordpress Cms.')
            sys.exit()
        else:
            pass

        sleep(0.3)
        print(color.CYAN+"[!] "+"Scanning 1000 best plugins...\n")
        sleep(4)
        print(color.LIGHTCYAN_EX+"[*] Loading...\n")
        sleep(2)

        for target in pl:
            sleep(0.1)
            try:
                checker = requests.get('http://'+site+'/wp-content/plugins/'+target)
                if checker.status_code == 200:
                    print(color.GREEN+"[+] "+color.BLUE+'http://'+site+"/wp-content/plugins/"+target+color.WHITE+" >"+" Found...")
                else:
                    print(color.RED+"[-]"+color.BLUE+"http://"+site+"/wp-content/plugins/"+target+" >"+" Not Found...")
...
def user_finder():
    try:
        ...
        print(color.CYAN+"[*] Please Do not Include Http in the url\n")
        sleep(0.1)
        site = input(color.RED+"┌─["+color.LIGHTGREEN_EX+"WebRobber"+color.RED+"/"+color.WHITE+"Home"+color.RED+"/"+color.CYAN+"CMS"+color.RED+"/"+color.LIGHTYELLOW_EX+"WP"+color.RED+"/"+color.CYAN+"User-Finder"+"""]
└──╼ """+color.WHITE+"» ")

        data = requests.get('http://'+site+'/wp-content/plugins')
        if data.status_code == 404 or data.status_code == 500 or data.status_code == 403:
            print(color.RED+"[!] This website is not Based on Wordpress Cms.")
            sys.exit()
        else:
            pass
            

        print(color.GREEN+"[*] Loading...")
        sleep(0.5)
    except:
    ...
    try:
        req = requests.get('https://'+site+'/wp-json/wp/v2/users/').text
        dict = json.loads(req)
        ...
        for val in dict:
            ...
            u = dict[cn]['slug']
            if not u == '':
                user += color.GREEN+'[+] '+color.CYAN+u+split
            cn += 1
        if user == '':
            user = color.RED + "Nothing Found :("

        print(user+'\n')
    
    except:
        print(color.BLUE+" [!] "+color.RED+"ERROR ! ! \n"+"\n"+color.BLUE+" [!] "+color.GREEN+"Please Enter The Valid Address\n"+"\n"+color.BLUE+" [!] "+color.LIGHTWHITE_EX+"Or Plase Check Your Connection\n"+"\n"+color.BLUE+" [!] "+color.YELLOW+"Or I could not find the usernames\n ")
        sys.exit()

    try:
        input(color.RED+" [!] "+color.GREEN+"Back To Menu (Press Enter...) ")
    except:
        print(color.RED+"Something bad happend....")
        sys.exit() 
