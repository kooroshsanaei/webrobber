...
def __start__():
    try:
        sleep(0.1)
        print(color.CYAN+"[!] Enter The Website Address\n")
        sleep(0.1)

        site = input(color.RED+"┌─["+color.LIGHTGREEN_EX+"WebRobber"+color.RED+"/"+color.WHITE+"Home"+color.RED+"/"+color.CYAN+"IG"+color.RED+"/"+color.LIGHTYELLOW_EX+"Admin-Finder"+color.RED+"""]
└──╼ """+color.WHITE+"» ")
        print(color.CYAN+"Loading...")
    #Set a check list for bruteforcing 

        bf_list = ['admin/','administrator/','login.php','administration/','admin1/','admin2/','admin3/','admin4/','admin5/','moderator/','webadmin/','adminarea/','bb-admin/','adminLogin/','admin_area/','panel-administracion/','instadmin/',
...
'adm/index.brf','adm.brf','affiliate.brf','adm_auth.brf','memberadmin.brf','administratorlogin.brf','cpanel','cpanel.php','cpanel.html',]
        ...
        for target in bf_list:
            data = requests.get(site+target)

            if (data.status_code == 200):
                print(color.GREEN+"[+] "+site+target+" Found")
            else:
                print(color.RED+"[-] "+site+target+" Not Found")
